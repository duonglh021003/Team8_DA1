...
public class HoaDonChiTietRepository {
    ...
    public Boolean delete(HoaDonChiTiet hd) {
        ...
        public boolean deleteAll(HoaDonChiTiet hdct) {
        Transaction transaction = null;
        try ( Session session = HibernateUtil.getFACTORY().openSession()) {
            transaction = session.getTransaction();
            transaction.begin();
            Query query = session.createQuery("delete from HoaDonChiTiet where IdHoaDon = :idHoaDon");
            query.setParameter("idHoaDon", hdct.getIdHoaDonChiTiet().getHoaDon());
            query.executeUpdate();
            transaction.commit();
            return true;
        } catch (Exception e) {
